...
$sql = "SELECT pc.pc_id AS id, u.hr_id requested, u1.hr_id approved, pc_addDate entrydate, pc_total total_amount, pc_attach attachfile
...
while ($row = mysqli_fetch_assoc($result)) {
	...
	<EntryDate>'.date("Y-m-d", $row['entrydate']).'</EntryDate>
	<TotalAmount>'.$row['total_amount'].'</TotalAmount>
            <AttachFile>'.$row['attachfile'].'</AttachFile>
    		</ROW_DATA>';
...
//					WHERE claimId = '".$row['id']."' AND status = 1";
    $sql = "SELECT pcd_id id, pcd_pc_id claimId, pcd_remark title, pcd_purposeDate purposedate, pcd_category category, pcd_subcategory subcategory, pcd_division division_id, pcd_subdivision subdivision_id, pcd_jobno jobno, pcd_amount amount, pc.pc_currency currency
    ...
    while ($row1 = mysqli_fetch_assoc($result_detail)) {
        ...
        $detail .= '<ROW_DATA>
			...
			<Date>'.date("Y-m-d", $row1['purposedate']).'</Date>
...
//				WHERE c.updated_to_erp = 0 AND c.claimstatus = 4";
    $sql = "SELECT pc.pc_id id,pc.pc_userid requested,pc.pc_sendto approved,pc.pc_addDate entrydate, pc.pc_total total_amount, pc_attach attachfile
			FROM tbl_pettycash pc 
				WHERE pc.updated_to_erp = 0 AND pc.pc_status = 2";
    $result = mysqli_query($con,$sql);
