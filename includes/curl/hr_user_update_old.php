...
$work_category = $_REQUEST['work_category'];
$marital_status = $_REQUEST['marital_status'];
$home_town = $_REQUEST['hometown'];
$address = $_REQUEST['address'];
$emp_division_category = $_REQUEST['emp_division_category'];
...
if (!empty($_REQUEST['doAction']) && $_REQUEST['doAction'] == "CANCELEMPLOYEE") {
    ...
    } else if (count($arrClidren) > 0 && !empty($arrClidren)) {
	...
	$sql_upd = "UPDATE tbl_users SET division_head=".$row_head['user_id']. " WHERE user_id=".$row['user_id'];
	$mysqli->query($sql_upd);
...
//Set who the message is to be sent from
	foreach($arrr_data as $key => $data) {
		...
		$mail->AddCC("software@ariesgroup.ae");
...
############## MIAL SENDING OPTION ##################################
	...
	/*$arrr_data[] = array("to_mail"=>$user_row['personal_email'],
		...
		"body" =>" <b>Dear ".$user_row['full_name']."</b>,<br/><br/>
				  ...
				  <b>Kind Regards,<br/><br/>Effism Team</b>",
		"cc"=>'software@ariesgroup.ae'.",".(!empty($user_row['head_email'])?$user_row['head_email']:$user_row['head_personal_email'])
		);
...
############## MIAL SENDING OPTION ##################################
	...
	home_town='$home_town',
	address='$address',
	emp_div_category='$emp_division_category',
	work_location='$work_location'";
        ...
        $result = mysqli_query($con, $sql);
		   ...
		   $user_name = trim($user_name);
             $sql = "insert into tbl_users(hr_id,username,full_name,	employee_code,designation,gender,dob,doj,gdoj,personal_email,mobile,personal_mobile,category,emp_company_id,emp_division_id, 	emp_subdivision_id,group_id,emp_type,emp_category,reporting_location,work_location,status,is_regular,photo,date_created,user_height,blood_group,is_health, 	is_erp,work_category,marital_status,home_town,address,emp_div_category,reporting_time) values ('$hr_id','$user_name','$full_name','$employee_code','$designation','$gender','$dob','$doj','$gdoj','$personal_email','$mobile','$personal_mobile','user','$emp_company_id','$emp_division_id', '$emp_subdivision_id','$group_id','$emp_type','$emp_category','$reporting_location','$work_location','Active',0,'$photo','$date_created','$user_height','$blood_group',0,1,'$work_category','$marital_status','$home_town','$address','$emp_division_category','$reporting_time')";
            $email = "";
        ...
        $result = mysqli_query($con, $sql);
            ...
            $body = "Hi $full_name, <br/><br/> <b style='color:red;'>Your Efficiency Account has been created successfully!</b><br/><br/>
			 Click this link to activate the account&nbsp;&nbsp;&nbsp;<a href='http://www.effism.com/usernamecreation.php?$dta'>http://www.effism.com/usernamecreation.php?" . $dta . "</a>";
            //Create a new PHPMailer instance
            $mail = new PHPMailer(true);
//Tell PHPMailer to use SMTP
            $mail->isSMTP();
